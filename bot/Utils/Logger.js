...
module.exports = {
    ...
    writeDebug(client, userId, data) {
        ...
        if (c.database && c.database.models && c.database.models.Client && c.user) {
            c.database.models.Client.isInDebugMode(c.user.id).then(debugMode => {
                if (debugMode === true && c.settings.bot.developer.includes(userId)) {
                    const date = new Date();
    ...
    debug(source, content, client) {
        ...
        if (c.database && c.database.models && c.database.models.Client && c.user) {
            c.database.models.Client.isInDebugMode(c.user.id).then(debugMode => {
                if (debugMode === true) {
                    console.debug(`[${time()}] ${c.modules.chalk.magentaBright("DEBUG")} [${source}]: ${content}`);
                }
            });
        } else {
            console.debug(`[${time()}] ${c.modules.chalk.magentaBright("DEBUG")} [${source}]: ${content}`);
